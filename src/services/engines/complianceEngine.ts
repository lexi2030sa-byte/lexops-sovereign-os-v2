...
export const complianceEngine: SovereignEngine = {
  ...
  async execute(payload: any, ctx: EngineContext): Promise<EngineResult> {
    ...
    try {
      const metrics = await getComplianceMetrics(ctx.entityId);
      return {
        success: true,
        data: metrics,
        timestamp: ts,
